...
GO
CREATE OR ALTER FUNCTION dbo.tmpGetContextChangeNum() RETURNS INT AS BEGIN RETURN (SELECT CONVERT(INT, SUBSTRING(CONTEXT_INFO(), 2, 4))) END
GO
...
CREATE OR ALTER PROC #usp_CheckState (@T INT) AS BEGIN
    ...
    IF (@IsActive = 1) BEGIN
        ...
        END ELSE BEGIN
            EXEC #print 1, 'Failed: Transaction mismatch (', @@TRANCOUNT, ' <> ', @T, ')'
        END
end else begin exec #print 0, 'Not running'
    END
...
SET NOEXEC OFF; EXEC #SetCI 1, 0; EXEC #usp_CheckState 0; IF (dbo.tmpIsActive() = 0) SET NOEXEC ON
GO
BEGIN TRAN
GO");
        }
